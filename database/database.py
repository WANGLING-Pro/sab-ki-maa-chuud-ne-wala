...
class Database:
    ...
    async def full_userbase(self):
        data = await self.users.find({}, {"_id": 1}).to_list(None)
    ...
    async def get_ban_users(self):
        data = await self.banned.find({}, {"_id": 1}).to_list(None)
        return [x["_id"] for x in data]
    ...
    async def show_channels(self):
        data = await self.fsub.find({}, {"_id": 1}).to_list(None)
        return [x["_id"] for x in data]
    ...
    async def req_user_exist(self, channel_id: int, user_id: int):
        return bool(await self.req_fsub.find_one({"_id": channel_id, "users": user_id}))

    async def reqChannel_exist(self, channel_id: int):
        return bool(await self.req_fsub.find_one({"_id": channel_id}))
